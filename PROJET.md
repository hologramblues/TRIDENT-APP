...
- **L'Oracle — module de readings** (spec : `ORACLE-MODULE.md`, intégration en 4 phases). **Phase 1 livrée le 2026-08-14** : socle local complet, sans réseau ni IA. Tables `OR_ZODIAC` (12 signes + face d'ombre), `OR_TAROT` (22 arcanes, droit/envers), `OR_CHINOIS` + table des dates de Nouvel An chinois 1924-2026 (`OR_CNY` — sans elle une naissance de janvier donne un signe faux), `OR_EVENEMENTS` (repères de cohorte filtrés sur la fenêtre formative 8-25 ans), `OR_PRENOMS` (pic de popularité → l'écart avec la naissance est une information). Calculs : âge, signe, chinois, chemin de vie **avec maîtres nombres 11/22/33**, année personnelle, retour de Saturne, transitions de Levinson, stade d'Erikson, âge relatif scolaire. Mini-langage `orParse()` (`prenom JJMMAA thème± /détail`) avec rejet des dates impossibles (31 février, 29 février hors bissextile). Écrans saisie (décodage live + ligne de profil) et file one-ahead (fiche du haut distincte, dépliage, révélée/supprimer). Persistance `oracle:seance`, effacement complet — aucune donnée spectateur ne survit à la séance. Point d'entrée unique `window.oracleIngest(payload)` pour le pont natif et les futures apps de capture. Vérifié par exécution JS : bornes des 12 signes, 8/8 signes chinois dont bornes janvier/février, chemins de vie et maîtres nombres, dates impossibles, persistance après rechargement.
  **Phases suivantes** : 2) moteur IA via nouvelle commande de pont `apiCall` (le hub est en `file://`, CORS bloquerait un appel direct) + répertoire de codes + partition 14 caractères/ligne + banc A/B contre leurre Barnum · 3) mode scène (noir absolu, filtres, 3 niveaux, gestes, sortie par appui long coin) + PeekSmith + saisie aveugle · 4) modèles de billets + tirage tarot imposé formulé au futur.
